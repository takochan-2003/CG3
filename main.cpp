...
LRESULT CALLBACK WindowsProc(HWND hwnd, UINT msg,
...
{
	Log(ConvertString(std::format(L"Begin CompileSheder,path:{},profile:{}\n", filePath, profile)));
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//色
    float isColor[4] = { 1.0f,1.0f,1.0f,1.0f};
	...
	D3D12_INPUT_ELEMENT_DESC inputElemntDescs[1] = {};
	...
	inputLayoutDesc.pInputElementDescs = inputElemntDescs;
	inputLayoutDesc.NumElements = _countof(inputElemntDescs);
...
#pragma region 更新処理
			...
			*wvpData = worldMatrix;
			...
			if (ImGui::TreeNode("color")) {
				...
				ImGui::SliderFloat("A", alpha, 0.0f, 1.0f);
				
				ImGui::ColorEdit4("Color", isColor,1);
